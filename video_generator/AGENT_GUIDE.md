...
1. **Focus di Nicchia Assoluto:** Tutti i prompt (CLI in Bash e Python SDK in `rag_generator.py`) forzano l'agente a cercare notizie solo nei settori: *Mente, psicologia, salute, neuroscienze, nutrizione e crescita personale*.
2. **Memoria Anti-Duplicazione:** È stato introdotto il file di log `used_news_history.txt`. Prima di ogni generazione, il sistema inietta le ultime 20 notizie usate nel prompt dell'agente ordinandogli di ignorarle, impedendo che i task automatizzati peschino sempre le stesse news di tendenza. Alla fine della generazione, il titolo viene appeso al file.
3. **Modalità "Bastian Contrario":** È stata introdotta una logica contrariana (`video_bastian.sh` e `carousel_bastian.sh`). Usata con moderazione (es. una volta al giorno) serve a creare hook polarizzanti, smontando i luoghi comuni e spingendo gli utenti a commentare, aumentando vertiginosamente la visibilità.
