...
if WEB3_AVAILABLE:
    ...
    WETH_BASE = USDC_BASE = CHAINLINK_ETH_USD = UNIV3_ROUTER = UNIV3_FACTORY = AERO_ROUTER = AERO_FACTORY = ""
...
def _ensure_approval(token_addr: str, spender: str, amount_wei: int, owner: str) -> bool:
    """Approva spender se l'allowance è insufficiente. Ritorna True in DRY_RUN.
...
def execute_buy(signal_id: str, token_symbol: str, token_address: str,
    ...
    if not DRY_RUN:
        ...
        # Stima min_out: se abbiamo prezzo oracle usiamolo, altrimenti 0
        min_out = 0
        if tokens_est > 0:
...
def execute_sell(signal_id: str, sell_fraction: float, action_label: str,
    ...
    if tokens_sell <= 0:
        # Posizione senza token (DRY_RUN con oracle non disponibile) → marca closed per evitare spam
        pos["status"]   = "closed"
        pos["close_ts"] = datetime.now().isoformat()
        log.debug(f"[SELL] {signal_id}: tokens_held=0 → marcata closed (DRY_RUN senza oracle)")
        return True
...
def process_row(row: dict, processed: set, real_state: dict, token_lookup: dict) -> bool:
    ...
    if action == "entry":
        ...
        if sid not in real_state or real_state[sid].get("status") != "open":
            return False
        frac = 0.5 if action == "tp1" else 1.0
