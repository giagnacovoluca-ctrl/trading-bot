...
Aggiornato: 2026-07-30 (INTEGRAZIONE GRID BOT, FUNDING FARMER E UI TABS):

**Nuovi Sistemi Integrati:**
- **Grid Trading Bot** (`trade/grid_trader.py`, `trade/grid_config.py`): Sistema autonomo di grid trading con rilevamento automatico del range, circuit breaker (Max Drawdown e Max Volatility) e persistenza di stato su disco.
- **Funding Farmer Bot** (`injective_autopilot/funding_farmer_auto.py`): Sistema per automatizzare lo yield farming sfruttando i tassi di funding su Injective, in modalità PAPER trade continua.

**Miglioramenti UI e Filtri (`defi/trade_simulator.py`):**
- **Filtro DEFI BSR**: Abbassato da 1.20 a 1.0 (migliore cattura momentum) e ripristinata la logica anti-dump originale.
- **Dashboard UI (`sim_report.html`)**: Sostituita l'interfaccia lineare infinita con una navigazione a schede (Tab) custom (Vanilla JS/CSS). I tab includono: DEFI Simulator, Injective Autopilot, Grid Bot e Funding Farmer.
- Tutti i nuovi bot e il loop di orchestrazione `run.py` sono stati riavviati con le nuove configurazioni.

Aggiornato: 2026-06-28 (LIQ_MONITOR 3 FIX: honeypot alla radice + Jupiter sell sim Solana + filtro pool major):

**Fix 5 — liq_monitor: filtro honeypot alla radice (liquidity_event_monitor.py _tick() + _handle_pair_created()):**
- Problema: base_honeypot_symbols.json bloccava i simboli solo nell'executor (sell side).
  Scanner continuava a generare segnali → stessi token (COREX, ABS, Juno, GROOM…) riciclavano ogni tick.
- Fix: `_reload_honeypot_bl()` carica la blacklist da disco ogni 60s (recepisce aggiornamenti runtime).
  `_is_honeypot_sym()` controlla il simbolo PRIMA di scrivere pump_grad_signals.csv o shadow_queue.
  Applicato in entrambi i path: GeckoTerminal polling (_tick) e WebSocket Base factory (_handle_pair_created).
- Impatto: -90% segnali generati (161/h pre-fix → 16/h post-fix dalla 11:14 del 28/06).

**Fix 6 — liq_monitor: Jupiter sell simulation per Solana (_tick() ~riga 338):**
- Equivalente del sell simulato on-chain V2 già presente per Base (_handle_pair_created riga ~418).
- Prima di scrivere segnale Solana con liq≥$25k: chiama `_jupiter_sellable(token_address)`.
  GET https://api.jup.ag/swap/v1/quote inputMint=token, outputMint=USDC, amount=1M, slippage=50%, onlyDirectRoutes=true.
  Se outAmount=0 o 400 → skip con log "Jupiter no route — probabile honeypot".
- Cache 5 min per token_address per evitare call ripetute.
- Fail-open: API down → segnale passa (non bloccare per problemi transitori Jupiter).
- Simboli bloccati 28/06: USWR, GORP, America250, NTFS, AAIF, USA250, WYNN, MANIFEST, HERMES ecc.

**Fix 7 — liq_monitor: filtro pool major (_tick() ~riga 314):**
- Pool con liq>$500k → skip: token pump_grad legittimi entrano a $25k-$100k, non $500k+.
- Cattura falsi positivi residui: ㅤ ($125M-$2.7B), mɔ ($46M-$390M), WYNN ($16M).
- NON usare vol_h1==0 come filtro: pool nuovissime (<5min) hanno sempre vol_h1=0 per design.

**Risultati 28/06 post-fix (dalle 11:14):**
- Segnali Solana: 161/h → 16/h. Quelli che passano: tutti exit_time_limit avg -0.76€ (pool major residue).
- exit_liq_pump_would_hit shadow: 90 hit/24h avg +108.6% — pattern attivo, attende trigger live.
- Injective: 0 trade oggi (spread >80bps su tutti i ticker: PUMP 92-99bps, TON 103bps, TIA 83-86bps).
- Helius 429 gestito: rug_watcher + mirror_bot in backoff 300s automatico.
- base_honeypot_symbols.json: +6 simboli (ansem, octo, ornith, sleuth, staccana, tick).

Aggiornato: 2026-06-27 (EXECUTOR RUG PATTERN + 3 FIX: liq_collapse_lost, PumpSwap fallback, exit_liq_pump):

**Stato executor (27/06):**
- DRY_RUN_SYSTEMS=pump_grad,pre_grad,defi,v3,v3_midcap,v2,bnf,mirror → TUTTI in dry
- In DRY_RUN chiama ugualmente Jupiter/Raydium per quote reali → prezzi on-chain autentici
- Ultimo trade reale: 01/06 (110$ deployati, -46.91$ netto, bug sell non eseguita su 5 pos)
- Performance DRY_RUN 24h (prezzi Jupiter): 42W/9L +$631 su chiusi, 7 stuck -$700 → netto -$69

**Pattern honeypot identificato (n=23 rug liq_velocity, 24h):**
- Tutti i rug avevano prev_liq $33k-$62k (avg $51k) al momento del drain
- Entry liq: $15-27k → pool CRESCE a $33-62k (creator aggiunge LP) → drain a $0 in 10s
- Nessun token vincente ha mai visto pool >$30k prima del TP1
- Timeline: entry → pump 10-20 min → drain → liq_collapse_stuck (impossibile vendere)

**Fix 1 — executor: liq_collapse_lost (solana_executor.py execute_sell() ~riga 1035):**
- Quando no-route E action_label=="liq_collapse": chiude immediatamente a -100% (return True)
- Ordine sell fallback: Raydium → Jupiter (50% slippage) → PumpSwap → liq_collapse_lost
- Prima: stuck per ore (3 tentativi, posizione aperta per sempre, -100$ non contabilizzati)
- `_ps_result = None` dichiarato prima del blocco; DEX exec wrappato con `if _ps_result is None`

**Fix 2 — executor: PumpSwap fallback sell (solana_executor.py ~riga 1038):**
- `_pumpswap_sell()` già implementata (usata solo nel buy), ora tentata anche nel sell
- Solo live (not dry) + sistema pump_grad/pre_grad o signal_id.startswith("LIQ_")
- Se PumpSwap ok: converti SOL→USDC con _get_sol_price_usd(), log come "sent"

**Fix 3 — simulator: exit_liq_pump (trade_simulator.py _process_position() ~riga 2936):**
- Trigger: cur_liq > 25_000 AND cur_liq > entry_liq * 1.8 AND NOT in_grace
- Solo pump_grad/pre_grad. Posizione: DOPO liq_velocity, PRIMA di liq_collapse assoluta
- Esce mentre pool ha $25-60k liq → Jupiter può eseguire → evita stuck
- Cooldown: 24h in _COOLDOWN_MAP (come liq_collapse)
- Stima impatto: 23 rug → exit in profitto (+$20-40) invece di -$100 → netto stimato +$484/24h

**Fix 4 — shadow: exit_liq_pump_would_hit (trade_simulator.py _process_shadows() ~riga 1688):**
- Shadow ora fetchano cur_liq da _fetch_price()[3] (già disponibile)
- Stessa logica exit_liq_pump → exit_reason="exit_liq_pump_would_hit"
- entry_liq salvato in _shadow_register() da row["liquidity_usd"]
- Permette confronto live vs shadow su rug pattern

**Parametri chiave exit_liq_pump (da calibrare):**
- Soglia moltiplicatore: 1.8× (dati 27/06: rug range 1.5-4× entry_liq)
- Soglia assoluta min: $25,000
- Grace period: ENTRY_GRACE_MIN (2 min default)
- Rivalutare soglia se si vedono falsi positivi (win tagliati prima di TP1)
